...
parser.add_argument('--tag', default='GHGNN-train',
...
def graph_loss(V_pred, V_target, CVM):
    return bivariate_loss(V_pred, V_target, CVM)
...
def train(epoch, model, optimizer, checkpoint_dir, loader_train):
    ...
    for cnt, batch in enumerate(loader_train):
        ...
        CVM = V_obs[:, -1:, :, 1:]   # CVM [1, 1, N, 2-(x,y)]

        V_pred = model(V_obs, identity, obs_traj)  # A_obs <8, #, #> T N
        V_pred = V_pred.squeeze()
        V_tr = V_tr.squeeze()

        if batch_count % args.batch_size != 0 and cnt != turn_point:
            l = graph_loss(V_pred, V_tr, CVM)
            if is_fst_loss:
                loss = l
                is_fst_loss = False
            else:
